...
window.onload = function(){ 
	...
	function recordDelete(recordId) {
		if(confirm("您确定要删除这条记录吗？")) {
			window.location="repairManager?action=delete&recordId="+recordId;
		}
	}
</script>
...
<div class="data_list">
		<div class="data_list_title">
			<font style="color:white">寝室报修</font>
		</div>
		<form name="myForm" class="form-search" method="post" action="repairManager?action=search" style="padding-bottom: 0px">
				<span class="data_search">
				<button class="btn btn-success" type="button" style="margin-right: 150px;" onclick="javascript:window.location='repairManager?action=preSave'">添加</button>
			...
			<table class="table table-bordered datatable">
				...
				<c:forEach  varStatus="i" var="record" items="${recordList }">
					<tr style="color:white">
						...
						<td>${record.detail }</td>
						<td>
						<a href="http://beyond.3dnest.cn/play/?m=zqkj"><button class="btn btn-mini btn-success" type="button">维修</button>
						<button class="btn btn-mini btn-success" type="button" onclick="javascript:window.location='repairManager?action=preSave&recordId=${record.recordId }'">修改</button>
