...
import com.WangD.dao.RepairRecordDao;
...
public class RepairManagerServlet extends HttpServlet{
	...
	RepairRecordDao recordDao = new RepairRecordDao();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if("admin".equals((String)currentUserType)) {
				List<RepairRecord> recordList = recordDao.recordList(con, record);
				request.setAttribute("dormBuildList", recordDao.dormBuildList(con));
				request.setAttribute("recordList", recordList);
				request.setAttribute("mainPage", "admin/repairManager.jsp");
				...
				List<RepairRecord> recordList = recordDao.recordListWithBuild(con, record, buildId);
				request.setAttribute("dormBuildName", buildName);
				request.setAttribute("recordList", recordList);
				request.setAttribute("mainPage", "dormManager/repairManager.jsp");
				request.getRequestDispatcher("mainManager.jsp").forward(request, response);
			} else if("student".equals((String)currentUserType)) {
				Student student = (Student)(session.getAttribute("currentUser"));
				List<RepairRecord> recordList = recordDao.recordListWithNumber(con, record, student.getStuNumber());
				request.setAttribute("recordList", recordList);
				request.setAttribute("mainPage", "student/repairManager.jsp");
	...
	private void recordDelete(HttpServletRequest request,
		...
		try {
			...
			request.getRequestDispatcher("repairManager?action=list").forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	private void recordSave(HttpServletRequest request,
		...
		RepairRecord record = new RepairRecord(studentNumber, date, detail); 
		...
		try {
			...
			if(student.getName() == null) {
				...
				request.setAttribute("mainPage", "repairManager/repairRecordSave.jsp");
				request.getRequestDispatcher("mainManager.jsp").forward(request, response);
			} else {
	...
	private void recordPreSave(HttpServletRequest request,
		...
		try {
			con = dbUtil.getCon();
			if (StringUtil.isNotEmpty(recordId)) {
				RepairRecord record = recordDao.recordShow(con, recordId);
				request.setAttribute("record", record);
			} else {
		...
		request.setAttribute("mainPage", "dormManager/repairRecordSave.jsp");
		request.getRequestDispatcher("mainManager.jsp").forward(request, response);
	}
